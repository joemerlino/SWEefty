...
\end{lstlisting}
			...
			\item \textbf{Indentazione:}\textcolor{red}{CORRETTO: prima si "consigliava " di impostare gli spazi nell'IDE, invece deve essere una norma, nessuno deve fare altrimenti. } vengono utilizzati quattro spazi. L'$IDE_G$ utilizzato deve essere adeguatamento impostato per l'inserimento automatico di quattro spazi. Inoltre non sono permessi spazi bianchi o tabulazioni a fine riga;
...
*/
			...
			\textcolor{red}{DA CORREGGERE: devi rifarti ad \ref{versionamento}, scopiazza ma ricorda che li si parla di documenti e qui di codice.}
			...
			\begin{itemize}
				\item \textbf{X:} se sono stati introdotti dei $major\text{ }improvement_G$, l'incremento di questo numero significa che sono state aggiunte funzionalità importanti \textcolor{red}{DA CORREGGERE: esempi?}
				\item \textbf{Y:} se sono stati introdotti dei $minor\text{ }improvement_G$, $bugfix_G$. Va incrementato in caso di implementazione di funzionalità più piccole \textcolor{red}{DA CORREGGERE: esempi?}
				\item \textbf{Z:} se sono stati corretti $typos_G$, introdotti errori nei commenti o risolti piccoli $bug_G$
			\end{itemize}
	...
	\label{sec:Atom}
	\Spazio
	\textcolor{red}{DA CORREGGERE: sezione da approfondire molto: due punti principalmente, 1 - come si integra con le tecnologie che utilizziamo? 2 - quali sono questi plugin? perchè piacciono al gruppo? }
	Atom è un IDE di ultima generazione, sviluppato da GitHub. È un editor potente,
	$crossplatform_G$ e si integra perfettamente con tutte le tecnologie e strumenti che si utilizzano per lo sviluppo del prodotto. 
