...
\end{itemize}
	...
	SWEefty inoltre distingue le anomalie inviduate durante la fase check in quattro diverse categorie. 		Cio' permette di discutere piu' facilmente delle correzioni da atturare durante le riunioni. Le 			caegorie utilizzate sono quelle previste dal documento IEEE 610.12-90:

	\begin{itemize}
	\item \textbf{Error:} diffrenza tra un valore o una condizione computata, osservata e misurata e il valore "teorico" corretto aspettato. (e.g: la differenza tra la stima di un costo per una determinata attivita' ed il suo costo effettivo);

\item \textbf{Fault:} un processo, un'operazione o un dato definito in maniera erronea (e.g: violazione di un regola di formattazione in un documento);
	...
	\subsection{Gestion qualita' di processo}
Esistono due approcci per il controllo della qualita' di processo:
\begin{itemize}
	\item \textbf{Agile: } sviluppo iterativo senza l'overhead della documentazione. Permette uno sviluppo rapido e un'ottima capacita' di adattamento in caso di modifica dei requisiti da parte del fornitore;
	
	\item \textbf{A maturita' di processo: } previsto dalle buone pratiche di management, mette al primo posto la qualita' del prodotto e dei processi.
\end{itemize}
	SWEefty utilizzera' il primo secondo approccio in quanto piu' adatto ad un gruppo inesperto.
	
Dal punto di vista pratico il processo di miglioramento continuo che SWEefty utilizza (PDCA) verra' attuato nella sequente maniera:
\begin{itemize}
	\item la fase check verra' effettuata i giorni precedenti alle consengne del materiale peviste dal committente e verranno sollevate tutte le possibili problematiche riscontrate nelle due fasi precedenti (Plan, Do);
	\item la fase Act verra' effettuata a partire dal processo incrementale successivo. Successivamente si procedera' con le fasi Plan e Do.
\end{itemize}
