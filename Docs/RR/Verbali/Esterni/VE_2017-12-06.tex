...
\restoregeometry
	...
	\subsection{Argomenti affrontati}
     Nel corso della riunione è stato approfondito il capitolato, presentando le diverse tecnologie da apprendere per poterlo sviluppare.
     
     Gli argomenti principali discussi sono:
     \begin{itemize}
     	\item { \textbf{Kibana:} è stata presentata la piattaforma e ne sono state analizzate le diverse funzionalità.
     	Questa è la piattaforma open source progettata per rappresentare in modo grafico i dati ottenuti dopo averli richiesti al database.
        L'obiettivo del nostro team sarà appunto quello di realizzare un plugin che permetta di visualizzare all'interno di questa piattaforma dei dati di monitoraggio di un'applicazione.}
     	\item { \textbf{ElasticSearch:} è stato presentato questo nuovo tipo di database, che sarà il luogo nel quale vengono memorizzate le informazioni monitorate dall'applicazione. Sarà possibile ricercarle e raggrupparle mediante query.
     	Sono state affrontate le diversità di questa base di dati rispetto ad un database relazionale, ed è quindi stato spiegato in che modo le informazioni vergono organizzate all'interno di questo database.
        Dalla riunione è emerso inoltre, che le informazioni vengono gestite come documenti JSON. E' stato illustrato questo tipo di formato Javascript, ne sono stati fatti alcuni esempi inerenti al capitolato spiegandone l'utilità.}
     \end{itemize}
