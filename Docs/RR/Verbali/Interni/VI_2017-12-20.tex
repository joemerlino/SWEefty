...
\restoregeometry
    ...
    Nel corso della riunione sono stati approfonditi i seguenti punti stilati dal responsabile di progetto:
    
   
    \begin{itemize}
    \item { \textbf{Visionamento verifica del documento:} il documento
    \emph{NormeDiProgetto\_v1.0.0} è stato verificato dall'incaricato. Durante la riunione sono stati evidenziati i vari errori e le sezioni da sistemare/riformulare;}
	...
	\item { \textbf{Versione documento:} Si è deciso di inserire per ogni documento la versione dopo il suo titolo per esempio: \textit{NormeDiProgetto DA MOD};}
	\item { \textbf{Verbali:} si è deciso di riscrivere i verbali in modo formale all'interno di un documento. Sarà dunque necessario realizzare un template da utilizzare per scrivere i verbali. Il task è stato assegnato a Joe Merlino. Per quanta riguarda la stesura dei verbali, se ne occuperanno Elia Montecchio e }
