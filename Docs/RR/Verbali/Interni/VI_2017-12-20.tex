...
\restoregeometry
    ...
    \begin{itemize}
    \item { \textbf{Visionamento verifica del documento:} il documento \emph{NormeDiProgetto\_v1.0.0} è stato verificato dall'incaricato. Durante la riunione sono stati evidenziati i vari errori e le sezioni da sistemare/riformulare;}
	\item { \textbf{Discord:} strumento che il team utilizzerà per confrontarsi e per le riunioni durante il periodo delle vancaze natalizie, il quale è molto interessante in quanto permette anche di condividere tra i vari partecipanti lo schermo del proprio pc;}
	\item { \textbf{Versione documento:} Si è deciso di inserire per ogni documento la versione dopo il suo titolo per esempio: \textit{NormeDiProgetto\_v1.0.0};}
	\item { \textbf{Glossario:} si è deciso di glossarizzare i termini in modo automatico mediante uno script.
	...
	Abbiamo discusso ed approvato che il plugin riguardante la mappa topologica deve prevedere la possibilità di visualizzare a schermo intero il suo contenuto.
 
	Per quanto riguarda il plugin riguardante la visualizzazione delle stack trace è stato deciso di realizzare l'ordinamento richiesto in base al tempo di esecuzione e al momento in cui è iniziata l'esecuzione della trace. Entrambe devono poter avvenire in ordine crescente e decrescente.
		
	Abbiamo inoltre discusso della visualizzazione della lista delle queries di un call tree. Di esse dovranno essere mostrate le istruzioni che le compongono. Anche esse devono poter essere riordinabili per tempo di esecuzione e momento di inizio.
