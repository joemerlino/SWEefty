...
\restoregeometry
    ...
    Nel corso della riunione sono stati approfonditi e discussi i seguenti punti:
    
   
    \begin{itemize}
    \item { \textbf{Modulo A/B del capitolato:} Il capitolato propone due sottoparti ognuna delle quali sarà implementata da due team differenti. Durante l'incontro è stato deciso che il nostro team si occuperà di sviluppare il modulo B;}
	\item { \textbf{Strumenti di comunicazione team:} Si è deciso che lo strumento che il team utilizzerà per le comunicazioni sarà \textit{Slack}. Lo strumento sarà suddiviso in diversi canali:	
		\begin{itemize}
			\item { \textbf{general:} Per le comunicazioni informali;}
	...
	\item { \textbf{Diario:} Nell'intestazione del documento sarà presente il diario, nel quale verranno segnate in modo formale le modifice apportate al documento da ogni membro del gruppo.
	Per effettuare ogni modifica è necessario riportare autore,ruolo,data e versione del documento;}
	\item { \textbf{Diagrammi UML:} Nel corso della riunione è stato assegnato a Joe Merlino di cercare, per poi discutere nella prossima riunione un software per disegnare questo tipo di diagrammi;}
	\item { \textbf{Stesura Norme di Progetto:} Si è deciso di iniziare a stendere questo documento. La suddivisione delle sezioni da scrivere è stata suddivisa nel seguente modo: }
		\begin{itemize}
