...
\subsection{Sviluppo}
	...
	\subsubsection{Attività}
		...
		\paragraph{Progettazione}
			...
			Il responsabile di questa attività è il \emph{\gl{Progettista}} che, in funzione dei requisiti delineati nell'\textit{Analisi dei Requisiti}, deve produrre la \textit{Technology Baseline} e la \textit{Product Baseline} in modo da permettere di definire le linee guida da seguire durante l'attività di codifica.
			...
			\label{sec:tec}
			\Spazio
			Questo documento fa parte della consegna RP (Revisione di Progettazione) e deve essere presentato e discusso in stile agile in finestre di opportunità che precedono la revisione, presenta le tecnologie, i \gl{framework} e le librerie selezionate per lo sviluppo del prodotto e ne dimostra l'adeguatezza e il grado di integrazione tramite \textit{Proof of Concept} ovvero un prototipo eseguibile che dimostri ciò che il gruppo ha intenzione di realizzare, il codice del \textit{PoC} può essere usa e getta. \\
			...
			\label{sec:prod}
			\Spazio
			Questo documento fa parte della consegna RQ (Revisione di Qualifica) e deve essere presentato e discusso in stile agile in finestre di opportunità che precedono la revisione deve descrivere la baseline architetturale del prodotto dimostrando la coerenza con quanto mostrato nella \textit{Technology Baseline}.\\
