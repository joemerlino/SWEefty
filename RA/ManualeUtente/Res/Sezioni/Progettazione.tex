...
Il \gl{progetto} Havana si presenta come una coppia di plugin per Kibana. Per questo motivo è necessario predisporre di Kibana correttamente installato e configurato con \gl{Elasticsearch}.
...
Questo plugin si presenta come una mappa nella quale vengono visualizzati i nodi \gl{hardware} che compongono l'applicazione monitorata.
...
\subsubsection{Ingrandimento fullscreen}
\label{sec:zoom}
...
\label{sec:mail}
Se vengono riscontrati errori o problematiche di vario genere si prega di segnalarli nell'apposita sezione \gl{\emph{Issues}} della \gl{repository} \gl{GitHub} associata al prodotto Havana, oppure all'indirizzo e-mail sweeftyteam@gmail.com inserendo come oggetto \textbf{Bug report Havana} e seguendo il seguente template per il corpo della mail:
 \begin{itemize}
	\item \textbf{ElasticSearch version:} versione installata di ElasticSearch;
	\item \textbf{Kibana version:} versione installata di Kibana;
	\item \textbf{Plugin name:} nome del plugin Havana avente il problema;
	\item \textbf{Plugin version:} versione del plugin avente il problema;
	\item \textbf{Bug report:} piccolo riassunto dove è richiesto di evidenziare i passi per poter riprodurre il problema.
\end{itemize}
