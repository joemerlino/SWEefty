...
\newcommand{\Titolo}{Verbale riunione 9 Marzo 2018}
...
\restoregeometry
	...
	\section{Informazioni generali}
		\subsection{Informazioni incontro}
			...
			\begin{itemize}
				...
				\item { \textbf{Data:} 9 Marzo 2018 }
				\item { \textbf{Ora inizio:} 14.33 }
				\item { \textbf{Ora fine:} 14.51 }
    ...
    Nel corso della riunione effettuata mediante Skype, è stato mostrato all'azienda lo stato del Proof of Concept. Inoltre, sono state discusse varie problematiche emerse durante la programmazione della demo.
    Nello specifico:

    \begin{itemize}
	\item { Sono state illustrate al proponente le due demo riguardanti la stack trace e il plugin della mappa topologica. La mappa topologica attualmente presenta una piattaforma comprensiva di 4 nodi: due database e due server.  É stato mostrato inoltre che il plugin è in grado di comunicare con ElasticSearch effettuando una richiesta per ottenere i dati e che il JSON risultante viene interpretato ed elaborato graficamente in modo corretto. 
	Per quanto riguarda la stack trace, viene rappresentata una tabella contenente tutti i nomi dei metodi di una trace e il rispettivo tempo di esecuzione. Inoltre, è possibile visualizzare nella tabella il tempo di esecuzione totale della trace.}
	\item { Il team ha spiegato all'azienda che è stato necessario addottare delle pratiche di reverse engineering per poter comprendere le meccaniche di comunicazione tra Kibana ed Elastic Search a causa della poca documentazione messa a disposizione dal team di sviluppo di Kibana. Inoltre, le istruzioni per l'installazione e lo sviluppo dei plugin generati tramite il template ufficiale non erano complete ed aggionate pertanto è stato necessario un approfondimento autonomo da parte del gruppo su come interagire con Kibana.}
	\item { \textbf{01/VE\_2018-03-09} É stato deciso di utilizzare la libreria Javascript chiamata D3.js per rappresentare graficamente i componenti nella mappa topologia del plugin. }
	\item { Si è discusso inoltre di quale versione di Kibana sia opportuno installare nei server ufficiali di test e produzione, in quanto il team ha trovato difficoltà a sviluppare con la versione development di Kibana 6.2.2(stabile) preferendo utilizzare la versione 6.2.3. Dopo un breve confronto \textbf{02/VE\_2018-03-09} si è deciso di installare nel server la versione stabile di Kibana, effettuando dunque un rollback di quanto fatto finora in modo da renderlo compatibile con la versione precedente.}
	\item { L'azienda ci ha comunicato, che entro la giornata odierna, provvederà a comunicarci le credenziali ed il metodo per interagire con i server Amazon Web Services, che ci vengono messi a dispozione dal proponente per il test e la produzione.}
     \end{itemize}
