...
\newcommand{\Titolo}{Verbale riunione 20 Dicembre 2017}
...
% - "INSERT DATA"
\newcommand{\Data}{23 Dicembre 2017}
% - "INSERT DESCRIZIONE"
...
\restoregeometry
	...
	\section{Informazioni generali}
		\subsection{Informazioni incontro}
			...
			\begin{itemize}
				...
				\item { \textbf{Data:} 20 Dicembre 2017 }
    ...
    Nel corso della riunione sono stati discusse questioni inerenti all'organizzazione del gruppo e lo stato del lavoro in corso. Sono anche stati discussi i requisiti del progetto facendo anche riferimento alla riunione con IKS del 19 dicembre.
    
	\subsubsection{Organizzazione e stato del lavoro}
    \begin{itemize}
    \item { \textbf{Visionamento verifica del documento:} il documento \emph{NormeDiProgetto v1.0.0} è stato verificato dall'incaricato. Durante la riunione sono stati evidenziati i vari errori e le sezioni da sistemare/riformulare;}
	\item { \textbf{Discord:} 01/VI\_2017-12-20 Strumento che il team utilizzerà per confrontarsi e per le riunioni durante il periodo delle vancaze natalizie, il quale è molto interessante in quanto permette anche di condividere tra i vari partecipanti lo schermo del proprio pc;}
	\item { \textbf{Versione documento:} 02/VI\_2017-12-20 Si è deciso di inserire per ogni documento la versione dopo il suo titolo per esempio: \textit{NormeDiProgetto v1.0.0};}
	\item { \textbf{Glossario:} \textbf{ 03/VI\_2017-12-20 } si è deciso di glossarizzare i termini in modo automatico mediante uno script.
	I termini da glossarizzare non vanno più inseriti all'interno del documento, ma all'interno di un file denominato \textit{terminiGLossario.txt}. Ogni termine dev'essere inserito in ordine alfabetico. Inoltre, è stata realizzata una lista di file chiamati come le lettere dell'alfabeto, che rappresentano il nostro dizionario. E' necessario, dunque, inserire la parola all'interno del file chiamato con la lettere iniziale della parola in questione aggiungendo anche una descrizione del termine.}
    \end{itemize}
 
	\subsubsection{Requisiti}
	\textbf{ 04/VI\_2017-12-20 } Abbiamo discusso ed approvato che il plugin riguardante la mappa topologica deve prevedere la possibilità di visualizzare a schermo intero il suo contenuto.
 
	\textbf{ 05/VI\_2017-12-20 } Per quanto riguarda il plugin riguardante la visualizzazione delle stack trace è stato deciso di realizzare l'ordinamento richiesto in base al tempo di esecuzione e al momento in cui è iniziata l'esecuzione della trace. Entrambe devono poter avvenire in ordine crescente e decrescente.
		
	\textbf{ 06/VI\_2017-12-20 } Abbiamo inoltre discusso della visualizzazione della lista delle queries di un call tree. Di esse dovranno essere mostrate le istruzioni che le compongono. Anche esse devono poter essere riordinabili per tempo di esecuzione e momento di inizio.
