...
\subsection{Sviluppo}
	...
	\subsubsection{Attività}
	    ...
	    Per ogni livello i numeri partono dal numero uno e non hanno limite massimo. 
			...
			I diagrammi \gl{UML} devono essere realizzati usando la versione del linguaggio \textit{v2.0}. Essi forniscono una rappresentazione chiara e compatta dell'intera struttura dell'applicazione che si andrà a realizzare. In particolare i diagrammi che saranno realizzati potranno essere:
			\begin{itemize}
		...
		\paragraph{Progettazione}
			...
			\begin{enumerate}
				\item \textbf{In fase di sviluppo}: durante questa fase i \emph{Progettisti} lavorano attivamente sul diagramma che dunque ha una natura mutevole. Non trattandosi di un prodotto ufficiale o completo il diagramma, prodotto con il programma StarUML descritto in \ref{sec:StarUML}, deve essere salvato nella cartella all'interno dello spazio Dropbox denominata \texttt{UMLDiagrams}. I diagrammi delle classi vanno inseriti in una sottocartella denominata \texttt{Classes}, mentre quelli di sequenza vanno inseriti in una sottocartella chiamata \texttt{Sequences}. Il formato con il quale sono salvati i diagrammi è necessario permetta di modificarli in un secondo momento, dunque è opportuno che sia quello nativo di StarUML: \texttt{.mdj}.
				\item \textbf{Completato}: un diagramma che si trovi in questa fase può considerarsi completato. Esso è un prodotto completo ed ufficiale. Farà parte di un documento tecnico e dunque dovrà essere inserito all'interno di esso. Il formato naturale per salvare un diagramma \emph{completato} è sottoforma di immagine non modificabile, dunque di estensione \texttt{.png}. Tale immagine deve essere inserita all'interno della cartella del documento tecnico nella sottocartella \texttt{Images/UML}. Il nome con cui si salva tale immagine deve rendere chiaro ciò che il diagramma intende descrivere. Ad esempio un diagramma delle classi che descriva i componenti del lato backend dell'applicazione dovrebbe chiamarsi \texttt{backend.png}.
			\end{enumerate}
			...
			Per rappresentare al meglio le scelte prese si presenta un \emph{allegato tecnico} contenente:
				\begin{itemize}
					...
					devono essere descritti i design pattern adottati all'interno dell'architettura del prodotto tramite descrizione e relativo diagramma.
				\end{itemize}
		   ...
		   \begin{itemize}
		   	...
		   	i \emph{Progettisti} devono definire dei test per verificare che le unità del sistema si integrino nel modo previsto;
		   	\item \textbf{Test di unità:}
		   	i \emph{Progettisti} devono definire i test di unità necessari per verificare che le unità del sistema funzionino nel modo previsto.
		   \end{itemize}
...
*/
	...
	\label{sec:plugins}
	\Spazio
	Per sviluppare il prodotto il gruppo si avvale di una serie di plugin integrati nell'IDE. Essi sono stati selezionati appositamente per facilitare la produzione del \emph{prodotto}. Essi sono:
	
	\begin{itemize}
		\item \textbf{REST client}: Permette di eseguire chiamate \gl{REST} all'interno dell'ambiente \emph{Atom}, velocizzando il processo di \emph{testing} di esse per uno sviluppatore; 
		\item \textbf{Elasticsearch-client}: Permette di interrogare un database Elasticsearch all'interno di \emph{Atom}, senza dover utilizzare Kibana per ottenere una comunicazione semplice con esso; 
		\item \textbf{json-converter}: permette di convertire dati disponibili in alcuni formati (e.g. csv, yaml) in json, ovvero il formato utilizzato da Javascript per la rappresentazione degli stessi. 
	\end{itemize}

	Inoltre, ciascun membro del gruppo può avvalersi di ulteriori plugin che ritenga utili per la realizzazione del prodotto.
	
	\begin{figure}[H]
		\label{esplugin}
		\centering
		\includegraphics[width=1\textwidth]{Images/elasticsearchplugin.png}
		\caption{Plugin Elasticsearch-client per \emph{Atom}}
	\end{figure}
