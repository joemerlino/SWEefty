\section{Organigramma}
	\subsection{Redazione}
	\begin{table}[H]
	\centering
	\begin{tabular}{|p{4cm}|C{4cm}|C{5cm}|}
	\hline
		\thead{Nome} &\thead{Data} &\thead{Firma} \\
		Paolo Eccher & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/paolo.png} \\
		\hline
	\end{tabular}
	\caption{Redazione}
	\end{table}
	
	\subsection{Approvazione}
	\begin{table}[H]
		\centering
		\begin{tabular}{|p{4cm}|C{4cm}|C{5cm}|}
			\hline
			\thead{Nome} &\thead{Data} &\thead{Firma} \\
			\hline			
			Francesco Parolini & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/freppo.png}  \\
			\hline
			Tullio Vardanega &  &  \\
			\hline
			Riccardo Cardin &  &  \\
			\hline
		\end{tabular}
		\caption{Approvazione}
	\end{table}
	
	\subsection{Accettazione componenti}
		\begin{table}[H]
		\centering
		\begin{tabular}{|p{4cm}|C{4cm}|C{5cm}|}
			\hline
			\thead{Nome} &\thead{Data} &\thead{Firma} \\
			\hline			
			Paolo Eccher & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/paolo.png}  \\
			\hline
			Alberto Gallinaro & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/alberto.png} \\
			\hline
			Giuseppe Merlino & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/giuseppe.png} \\
			\hline
			Elia Montecchio & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/elia.png} \\
			\hline
			Lisa Parma & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/lisa.png} \\
			\hline
			Francesco Parolini & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/freppo.png} \\
			\hline
			Davide Zago & 2018-03-22 & \includegraphics[width=2cm]{../../CommonImages/firme/davide.png} \\
			\hline
		\end{tabular}
		\caption{Accettazione componenti}
	\end{table}

	\subsection{Componenti}	
	\begin{table}[H]
		\centering
		\begin{tabular}{|C{2cm}|C{2.3cm}|C{7cm}|C{3cm}|}
			\hline
			\thead{Nome} & \thead{Matricola} &\thead{Indirizzo} &\thead{Ruoli} \\
			\hline			
			Paolo Eccher & 1121210 & paolo.eccher@studenti.unipd.it &  \textit{Amministratore, Analista, Verificatore} \\
			\hline
			Alberto Gallinaro & 1123348 & alberto.gallinaro.1@studenti.unipd.it  &  \textit{Analista, Verificatore} \\
			\hline
			Giuseppe Merlino & 1026694 & giuseppe.merlino.1@studenti.unipd.it & \textit{Responsabile, Analista, Verificatore} \\
			\hline
			Elia Montecchio & 1099273 & elia.montecchio@studenti.unipd.it & \textit{Amministratore, Analista, Verificatore} \\
			\hline
			Lisa Parma & 1121284 & lisa.parma@studenti.unipd.it & \textit{Amministratore, Analista, Verificatore} \\
			\hline
			Francesco Parolini & 1121990 & francesco.parolini.1@studenti.unipd.it  & \textit{Responsabile, Amministratore, Verificatore} \\
			\hline
			Davide Zago & 1126046 & davide.zago.5@studenti.unipd.it & \textit{Responsabile, Amministratore, Analista, Verificatore }\\
			\hline
		\end{tabular}
		\caption{Componenti}
	\end{table}
	
	