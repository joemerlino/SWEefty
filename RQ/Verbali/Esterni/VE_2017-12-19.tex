...
\restoregeometry
	...
	\begin{itemize}
		\item Deve essere possibile visualizzare sulla mappa il nome di ogni componente;
		\item Se un componente è un server bisogna mostrarne il linguaggio di implementazione;
		\item Se un nodo dell'applicazione è composto da più server (è un cluster) deve essere possibile visualizzarne il numero.
	\end{itemize}
	
	Per quanto riguarda la visualizzazione delle interazioni tra componenti:
	\begin{itemize}
		\item Deve essere possibile visualizzare il tempo medio di risposta che esiste tra due componenti che interagiscono;
		\item Deve essere possibile distinguere il tipo di richieste che avvengono tra i componenti, che potrebbero ad esempio essere richieste HTTP.
	\end{itemize}
	
	In aggiunta ci è stato richiesto di poter interagire con la mappa. Deve essere possibile riposizionare i componenti nello spazio e deve essere presenta la funzionalità per ridisporli nella posizione di partenza.
	
	\subsubsection{Affinamento dei requisiti Stack Trace}
	
	Per l'altro plugin da realizzare ovvero quello per visualizzare la lista delle trace è stato concordato che la visualizzazione da realizzare deve essere riordinabile in base a dei parametri significativi.
	
    Per quanto riguarda la visualizzazione del call tree relativo ad una singola stack trace ci è stata richiesta la visualizzazione nel call tree delle queries a database effettuate da ogni metodo.

	Inoltre, per ogni call tree, deve essere visualizzata una lista di tutte le queries effettuate.
	
	\subsubsection{Requisiti generali}
	È stato richiesto che vi sia un disaccoppiamento fra logica applicativa e le tecnologie utilizzate, in questo modo potrà essere possibile cambiare, per esempio, la base di dati dove le informazioni vengono memorizzate senza dover rivoluzionare il software.
