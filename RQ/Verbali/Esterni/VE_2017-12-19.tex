...
\restoregeometry
    ...
    \begin{itemize}
	\item { Affinamento dei requisiti }
	\item { Spiegazione contenuto trace }
	\item { Tecnologie obbligatorie per il progetto}
     \end{itemize}
	
	\subsubsection{Affinamento dei requisiti Mappa Topologica}
	Per quanto riguarda il plugin che visualizza la mappa topologica dell'applicazione sono state concordate le aggiunte che seguono.
	
	Per quanto riguarda la visualizzazione dei componenti:
	\begin{itemize}
		\item \textbf{ 01/VE\_2017-12-19 } Deve essere possibile visualizzare sulla mappa il nome di ogni componente;
		\item \textbf{ 02/VE\_2017-12-19 } Se un componente è un server bisogna mostrarne il linguaggio di implementazione;
		\item \textbf{ 03/VE\_2017-12-19 } Se un nodo dell'applicazione è composto da più server (è un cluster) deve essere possibile visualizzarne il numero.
	\end{itemize}
	
	Per quanto riguarda la visualizzazione delle interazioni tra componenti:
	\begin{itemize}
		\item \textbf{ 04/VE\_2017-12-19 } Deve essere possibile visualizzare il tempo medio di risposta che esiste tra due componenti che interagiscono;
		\item \textbf{ 05/VE\_2017-12-19 } Deve essere possibile distinguere il tipo di richieste che avvengono tra i componenti, che potrebbero ad esempio essere richieste HTTP.
	\end{itemize}
	
	In aggiunta ci è stato richiesto di poter interagire con la mappa. \textbf{ 06/VE\_2017-12-19 } Deve essere possibile riposizionare i componenti nello spazio e deve essere presenta la funzionalità per ridisporli nella posizione di partenza.
	
	\subsubsection{Affinamento dei requisiti Stack Trace}
	
	\textbf{ 07/VE\_2017-12-19 } Per l'altro plugin da realizzare ovvero quello per visualizzare la lista delle trace è stato concordato che la visualizzazione da realizzare deve essere riordinabile in base a dei parametri significativi.
	
    \textbf{ 08/VE\_2017-12-19 } Per quanto riguarda la visualizzazione del call tree relativo ad una singola stack trace ci è stata richiesta la visualizzazione nel call tree delle queries a database effettuate da ogni metodo.

	Inoltre, per ogni call tree, \textbf{ 09/VE\_2017-12-19 } deve essere visualizzata una lista di tutte le queries effettuate.
	
	\subsubsection{Requisiti generali}
	\textbf{ 10/VE\_2017-12-19 } È stato richiesto che vi sia un disaccoppiamento fra logica applicativa e le tecnologie utilizzate, in questo modo potrà essere possibile cambiare, per esempio, la base di dati dove le informazioni vengono memorizzate senza dover rivoluzionare il software.
