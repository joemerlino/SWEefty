...
\texttt{ElasticsearchClient} è il componente che si occupa di fare le chiamate API al lato server. Non è una classe vera e propria all'interno del codice, ma un \emph{Service}. \textcolor{red}{SPIEGARE COSA È UN SERVICE}. Utilizzare un service è l'unico modo per poter eseguire chiamate REST parametrizzate al lato server. I metodi da esso esposti sono: 
\begin{itemize} 
...
La vera potenza delle direttive risiede nel fatto di essere innestabili tra loro,
...
Per renderizzare la stack trace sono state create due direttive.
\begin{itemize}
	\item \texttt{branch} la quale si occpua di generare dell' html per ogni oggetto che gli viene passato,
  		in branch viene inoltre specificata la funzione \texttt{link} che permette di gestire un caso particolare
 	\item \texttt{tree} il cui compito è di interare sugli oggetti di un array. Per ogni elemento viene richiamata la direttiva branch.
\end{itemize}
...
tag personalizzati che vengono poi interpretati dal parser di AngularJs.
Nel nostro caso, le direttive vengono usate per popolare la tabella delle stack trace
e per ogni trace la corrispondente sotto tabella consentendo di riutilizzare il codice. Lo scheletro della stack trace è dunque un documento html contenente delle direttive AngularJS. \textcolor{red}{qui descrivi brevemente come si relaziona lo scheletro HTML con i dati ottenuti da strackbuilder}
