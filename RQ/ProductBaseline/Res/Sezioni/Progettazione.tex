...
L'anello di congiunzione tra StackBuilder (la classe che genera i dati per la Stack) e le angularJs (piu' in specifico le direttive)
é rappresentato dalla variabile \texttt{$scope.nodes}, infatti i dati generati da StackBuilder vengono memorizzati su tale variabile e successivamente le direttive, utilizzano \texttt{$scope.nodes} come fonte dati.
